    ...
    <div className="app-shell">
      ...
      <main className="main-content">
        <header className="topbar">
          ...
          <label className="search-box"><Search size={17} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search people, movies, shows..." /></label>
        ...
        <section className="hero-section">
          <div className="hero-copy">
            ...
            <div className="hero-actions"><button className="primary-button" onClick={() => setShowRoom(true)}><Play size={17} fill="currentColor" /> Start Watching</button><button className="ghost-button" onClick={() => setActiveNav('Discover')}><Compass size={17} /> Explore Live</button></div>
          </div>
        ...
        <section className="section-block">
          ...
          <div className="activity-grid">
            {visibleActivities.map((activity) => <article className="activity-card" key={activity.id}><div className="poster-wrap"><img src={activity.image} alt={activity.title} /><div className="poster-gradient" /><div className="live-label"><span className="live-dot" /> LIVE</div><div className="poster-bottom"><span>{activity.time}</span><span>{activity.viewers} watching</span></div></div><div className="activity-info"><span className="avatar">{activity.avatar}</span><div className="activity-copy"><strong>{activity.name}</strong><span>is watching <b>{activity.title}</b></span></div><button className="tune-button" onClick={() => setShowRoom(true)}>Tune In</button></div></article>)}
          </div>
        </section>
